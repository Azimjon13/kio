<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="ДСМ" prop="title"   align="center" min-width="160px">
                <template slot-scope="scope">
                    <div class="dsmDowntimesClass" v-if="!scope.row.itSummary">
                        {{ scope.row.title }}
                    </div>
        ...
        <el-dialog title="Используемые коды в данном отчёте" width="60%" :visible.sync="showCodes" :before-close="closeCodes">
            ...
            >
                ...
                <el-table-column label="Для типа ДСМ" prop="title_technica_group" :min-width="16"></el-table-column>
            </el-table>
        </el-dialog>
...
<script>
import {dsmDatasByReport} from "@/api/dsms";
...
export default { 
    ...
    methods : {
        ...
        {
            const codesRes = await reasonsWithCodes(`/dsm`);
        ...
        async loadData() 
        {
            const res = await dsmDatasByReport(this.id_report, this.date);
        ...
        calculateDowntimes() 
        {
            ...
            // console.log(this.dsmsData);
            const dsms = this.dsmsData.map((e, i, s) => {
                if(s.length - 1 != i) 
                {
                    return { 
                        title: e.title, 
            ...
            const linkItog = dsms.at(-1).downtimes;
            ...
            const percentage = {
                ...
                downtimes: reportedReasonsList.reduce((acc, curr, i ) => {
                    acc[curr.id_reason] = { 
                        dayDowntime: Math.round(((linkItog[curr.id_reason].dayDowntime/this.dsmsData.at(-1).downtimes.summary.dayDowntime)*100)*10)/10,
                        sinceDowntime: Math.round(((linkItog[curr.id_reason].sinceDowntime/this.dsmsData.at(-1).downtimes.summary.sinceDowntime)*100)*10)/10,
                    }
                    return acc;
                }, {})
            }
            ...
            dsms.push(percentage);
            this.dsmDatasByReport = dsms;
        },
        ...
        excelGetData() {
            ...
            this.dsmDatasByReport.forEach((e, i, self) => {
                const currentDsm = [i+1, e.title, e.garage ?? ''];
                if(i >= self.length - 2)
                    currentDsm[0] = '';
                Object.keys(e.downtimes).forEach(key => {
                    currentDsm.push(e.downtimes[key].dayDowntime ? e.downtimes[key].dayDowntime : "0");
                    currentDsm.push(e.downtimes[key].sinceDowntime ? e.downtimes[key].sinceDowntime : "0");
                }); 
                currentDsm.push(e.workHour.day, e.workHour.since, e.kio.day, e.kio.since);
