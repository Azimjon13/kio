<template>
    <div class="app-container">
        <h3>Остановки ДСМ за сегодня {{ showTable ? "": "(скрыто)" }}
            ...
            <el-button 
                    ...
                    style="float: right;"
                    @click="openAdd"
                    v-if="checkPermission(['manage dsms'])"
        ...
        <div v-if="this.showTable">
            <el-form :model="downtimesFilter" label-position="top">
                <el-row :gutter="20">
                    ...
                    <el-col :span="4">
                        <el-form-item label="Фильтр по ДСМ" label-position="top">
                            <el-select 
                                v-model="downtimesFilter.selectedDsm" 
                                filterable
                                value-key="id_dsm"
                                :filter-method="dsmFilter"
                            >
        ...
        >
            ...
            <el-table-column :min-width="14" label="ДСМ" prop="title_dsm" align="center"></el-table-column>
            ...
            <el-table-column :min-width="8"  label="Действия" align="center">
                <template slot-scope="scope">
                    <el-button v-if="!scope.row.end_time && checkPermission(['manage dsms'])"  type="info" icon="el-icon-edit" circle @click="openEdit(scope.row.id_downtime)"></el-button>
                    <el-button v-if="checkPermission(['manage dsms'])" type="danger" icon="el-icon-delete" circle @click="remove(scope.row.id_downtime)"></el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <AddDowntime  v-if="checkPermission(['manage dsms'])" :show="showAdd" :close="closeAdd" :placeId="id_place" :date="date"></AddDowntime>
        <EditDowntime  v-if="editingDowntimeId && checkPermission(['manage dsms'])" :show="showEdit" :close="closeEdit" :placeId="id_place" :date="date" :downtimeId="editingDowntimeId"></EditDowntime>
    </div>
</template>


<script>
import {getDsmDowntimesDailiy, deleteDowntime} from '@/api/dsms';
...
export default {
    ...
    methods: {
        checkPermission,
        async loadDowntimes() {
            const { data } = await getDsmDowntimesDailiy(this.date, this.id_place);
            ...
            const dsms = data.downtimes.map(d => ({
                id_dsm: d.id_dsm, 
                title: d.title_dsm,
                garage: d.garage,
            }));

            this.dsmsList = dsms.filter((e,i,self) => i === self.findIndex((t) => t.id_dsm === e.id_dsm));
            this.dsmsListFiltered = this.dsmsList;
        ...
        paginate(p) 
        {
            if(this.downtimesFilter.selectedDsm || this.downtimesFilter.selectedReason || this.downtimesFilter.search.trim()){
                const s = this.downtimesFilter.search.trim();
                let filtered = this.downtimesList;

                if(this.downtimesFilter.selectedDsm)
                {
                    filtered = filtered.filter(r => r.id_dsm == this.downtimesFilter.selectedDsm.id_dsm);
                }
                ...
                if(s)
                {
                    filtered = filtered.filter(r => (r.code.indexOf(s) != -1 || r.garage.indexOf(s) != -1 || r.title_dsm.indexOf(s) != -1 || r.reason.indexOf(s) != -1));
                }
        ...
        dsmFilter(search)
        {
            this.dsmsListFiltered =  this.dsmsList.filter(r => (r.garage.indexOf(search) != -1 || r.title.indexOf(search) != -1 ));
        }, 
        reasonFilter(search)
        {
