<template>
    <el-dialog 
        title="Данные простоев по ДСМ с начала месяца" 
        width="70%" 
    ...
    >
        <div class="form-container">
            <el-descriptions direction="vertical" :column="4" border>
                ...
                <el-descriptions-item label="ДСМ">
                    {{ place.dsm }}
                </el-descriptions-item>
...
<script>
import { dsmDowntimes } from '@/api/dsms';
...
export default  {
    ...
    created() {
        this.loadDowntimesDsm();
    },
    methods:{
        async loadDowntimesDsm() {
            const {data} = await dsmDowntimes({id_place_dsm: this.id_place_dsm, date: this.date});
        ...
        excelHeader() 
        {
            ...
            const firstRow = ['', 'Площадка', '', 'ДСМ', '', 'Гараж №', 'На дату', ''];
            const secondRow = ['', this.place.place, '', this.place.dsm, '', this.place.garage, 'С: '+new Date(this.shifts.shift_month_start).toLocaleString(undefined, {year:'numeric', month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit'}).replace(',', ' '), ''];
            ...
            const merges = ['B1:C1',  // Первая строка Площадка на 2 колонки
                            ...
                            'D2:E3',  // Вторая строка ДСМ на 2 строки и на 2 колонки
