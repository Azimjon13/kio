<template>
    <el-dialog title="Добавление простоя ДСМ" width="50%" :visible.sync="this.show" :before-close="close">
        <div class="form-container">
            <el-form ref="addDowntimeDsmForm" :model="downtime" label-position="top" >
                <el-descriptions :column="1" border :label-style="{'min-width': '270px'}">
                    ...
                    <el-descriptions-item>
                        ...
                        <el-select
                            :disabled="true"
                            v-model="downtime.dsm"
                            placeholder="Введите для поиска ДСМ"
                            filterable
                            remote
                            reserve-keyword
                            :remote-method="searchDsmSelect"
                            :loading="loadingDsm"
                            value-key="id"
                        >
                            <el-option class="dsmOption" 
                                    v-for="dsm in dsmsList" 
                                    :key="dsm.id" 
                                    :label="dsm.title" 
                                    :value="dsm"
                            >
...
<script>
import {getDsmGroups, searchDsm, updateDowntime, getDowntime} from "@/api/dsms";
...
export default { 
    ...
    methods: {
        ...
        {
            let dateFrom = new Date(this.downtime.dsm.date_from).getTime() - 86400*1000;
            let dateTo = new Date(this.downtime.dsm.date_to).getTime();
        ...
        async searchDsmSelect(search)
        {
            if(search.trim() !== '' && search.length >= 2) 
            {
                this.loadingDsm = true;
                const query = {
                ...
                let {data} = await searchDsm(query);
                this.loadingDsm = false;
                if(data != null)
                {
                    this.dsmsList = data;
                }
        ...
        async saveDowntime()
        {
            const newDowntime = {
                id_place: this.downtime.id_place, 
                id_dsm: this.downtime.id_dsm, 
                id_place_dsm: this.downtime.id_place_dsm, 
            ...
            {
                this.$message({
                    message: 'Простой ДСМ успешно добавлен!',
                    type: 'success',
                    duration: 5 * 1000,
                });
                this.close();
