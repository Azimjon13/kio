<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="Экскаваторы" prop="title"   align="center" min-width="160px">
                <template slot-scope="scope">
                    <div class="excavatorDowntimesClass" v-if="!scope.row.itSummary">
                        {{ scope.row.title }}
                    </div>
        ...
        <el-dialog title="Используемые коды в данном отчёте" width="60%" :visible.sync="showCodes" :before-close="closeCodes">
            ...
            >
                ...
                <el-table-column label="Для типа экскаватора" prop="title_technica_group" :min-width="16"></el-table-column>
            </el-table>
        </el-dialog>
...
<script>
import {excavatorDatasByReport} from "@/api/excavators";
...
export default { 
    ...
    methods : {
        ...
        {
            const codesRes = await reasonsWithCodes(`/excavator`);
        ...
        async loadData() 
        {
            const res = await excavatorDatasByReport(this.id_report, this.date);
        ...
        calculateDowntimes() 
        {
            ...
            const excavators = this.excavatorsData.map((e, i, s) => {
                if(s.length - 1 != i) 
                {
                    return { 
                        title: e.title, 
            ...
            const linkItog = excavators.at(-1).downtimes;
            ...
            const percentage = {
                ...
                downtimes: reportedReasonsList.reduce((acc, curr, i ) => {
                    acc[curr.id_reason] = { 
                        dayDowntime: Math.round(((linkItog[curr.id_reason].dayDowntime/this.excavatorsData.at(-1).downtimes.summary.dayDowntime)*100)*10)/10,
                        sinceDowntime: Math.round(((linkItog[curr.id_reason].sinceDowntime/this.excavatorsData.at(-1).downtimes.summary.sinceDowntime)*100)*10)/10,
                    }
                    return acc;
                }, {})
            }
            ...
            excavators.push(percentage);
            this.excavatorDatasByReport = excavators;
        },
        ...
        excelGetData() {
            ...
            this.excavatorDatasByReport.forEach((e, i, self) => {
                const currentExcavator = [i+1, e.title, e.garage ?? ''];
                if(i >= self.length - 2)
                    currentExcavator[0] = '';
                Object.keys(e.downtimes).forEach(key => {
                    currentExcavator.push(e.downtimes[key].dayDowntime ? e.downtimes[key].dayDowntime : "0");
                    currentExcavator.push(e.downtimes[key].sinceDowntime ? e.downtimes[key].sinceDowntime : "0");
                }); 
                currentExcavator.push(e.workHour.day, e.workHour.since, e.kio.day, e.kio.since);
