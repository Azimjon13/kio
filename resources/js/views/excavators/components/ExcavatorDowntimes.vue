<template>
    <el-dialog 
        title="Данные простоев по экскаватору с начала месяца" 
        width="70%" 
    ...
    >
        <div class="form-container">
            <el-descriptions direction="vertical" :column="4" border>
                ...
                <el-descriptions-item label="Экскаватор">
                    {{ place.excavator }}
                </el-descriptions-item>
            ...
            <el-table
                ...
                ref="excavatorDowntimes"
                class="excavatorDowntimesTable"
...
<script>
import { excavatorDowntimes } from '@/api/excavators';

export default  {
    props: {
        show: Boolean,
        close: Function, 
        id_place_excavator: Number,
        date: String,
    },
    data() {
        return {
    ...
    created() {
        this.loadDowntimesExcavator();
    },
    methods:{
        async loadDowntimesExcavator() {
            const {data} = await excavatorDowntimes({id_place_excavator: this.id_place_excavator, date: this.date});
        ...
        excelHeader() 
        {
            ...
            const firstRow = ['', 'Площадка', '', 'Экскаватор', '', 'Гараж №', 'На дату', ''];
            const secondRow = ['', this.place.place, '', this.place.excavator, '', this.place.garage, 'С: '+new Date(this.shifts.shift_month_start).toLocaleString(undefined, {year:'numeric', month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit'}).replace(',', ' '), ''];
            ...
            const merges = ['B1:C1',  // Первая строка Площадка на 2 колонки
                            ...
                            'D2:E3',  // Вторая строка Экскаваторы на 2 строки и на 2 колонки
        ...
        excelGetData() {
            ...
            const data = this.downtimesFiltered.map((d, i) => {
                ...
                return [
                    i+1, 
                    d.type == 1 ? 'Плановый' : (d.type == 2 ? 'Внеплановый' : 'Эксп. вне объёма'), 
                    d.reason, 
                    d.code, 
                    d.title_code, 
