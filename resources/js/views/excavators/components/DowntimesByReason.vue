        ...
        <div class="form-container">
            <el-descriptions direction="vertical" :column="5" border>
                ...
                <el-descriptions-item label="Тип причины">
                    {{place.type == 3 ? "Эксп. вне объёма" : (place.type == 1 ? "Плановый":"Внеплановый")}}
                </el-descriptions-item>
            ...
            <el-select v-model="downtimesFilter.id_technica_group" @change="filterDowntimes('reset')">
                ...
                <el-option v-for="group in excavatorGroupTypes" :value="group.id_technica_group" :label="group.title_technica_type"></el-option>
            </el-select>


            <el-select 
                    v-model="downtimesFilter.id_excavator" 
                    @change="filterDowntimes"      
                    filterable 
                >
                <el-option class="excavatorOption all"
                    label="Все экскаваторы"
                    :value="null"
                >
                ...
                <el-option class="excavatorOption"
                    ...
                    :label="excavator.title_excavator"
                    :value="excavator.id_excavator"
                >
                    <div class="autocomplete-list-item">
                            <span class="name-autocomplete">
                                <b>Наименование:</b> {{ excavator.title_excavator }}
                            </span>
                            <br>
            ...
            >
                ...
                <el-table-column label="Тип экскаватора" prop="title_technica_type" :min-width="9" align="center"></el-table-column>
                <el-table-column label="Экскаватор" prop="title_excavator" :min-width="15" align="center"></el-table-column>
...
<script>
// import {downtimesByReasonPlace} from "@/api/excavators";
import {downtimesByReasonPlace} from "../../../api/excavators";
...
export default {
    ...
    methods: { 
        async loadData() {
            ...
            const groupsExcavators = data.downtimes.map(r=>({id_technica_group: r.id_technica_group, title_technica_type: r.title_technica_type}));
            this.excavatorGroupTypes = groupsExcavators.reduce((acc, curr, ind) => {
            ...
            const excavators = data.downtimes.map(r => ({id_excavator: r.id_excavator, title_excavator: r.title_excavator, title_technica_type: r.title_technica_type, id_technica_group: r.id_technica_group, garage: r.garage}));
            this.excavatorsList = excavators.reduce((acc, curr, ind) => {
                if(!acc.find(a => a.id_excavator === curr.id_excavator)){
                    acc.push(curr);
                }
                return acc;
            }, []);
        ...
        filterDowntimes(reset) {
            ...
            let excavators = this.excavatorsList;
            ...
            if(this.downtimesFilter.id_technica_group)
            {
                ...
                excavators = excavators.filter(e => e.id_technica_group == this.downtimesFilter.id_technica_group);
            }

            if(reset == 'reset')
            {
                this.downtimesFilter.id_excavator = null;
            }

            if(this.downtimesFilter.id_excavator) 
            {
                downtimes = downtimes.filter(d => d.id_excavator == this.downtimesFilter.id_excavator);
            }
            
            this.excavatorsListFiltered = excavators;
        ...
        {
            this.excavatorsListFiltered =  this.excavatorsList.filter(r => (r.garage.indexOf(search) != -1 || r.title_excavator.indexOf(search) != -1 ));
        }, 
        ...
        excelHeader() 
        {
            ...
            const secondRow = ['', 
                                    this.place.place, '', 
                                    this.place.type == '3' ? 'Эксп. вне объёма' :( this.place.type == 2 ? 'Внеплановый' : 'Плановый'), '', 
                                    this.place.reason,  '',
            ...
            const fourthRow = ['','','','','','','','', '']; 
            const fivthRow = ['№ п/п', 'Тип экскаватора', 'Экскаватор', 'Гараж №', 'Код', 'Описание кода простоя', 'Дата начала простоя', 'Дата окончания простоя', 'Длительность (в час)'];
            
            const merges = ['B1:C1',  // Первая строка Площадка на 2 колонки
                            'D1:E1',  // Первая строка Экскаватор на 2 колонки
