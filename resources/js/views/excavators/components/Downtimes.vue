<template>
    <div class="app-container">
        <h3>Остановки экскаваторов за сегодня {{ showTable ? "": "(скрыто)" }}
            ...
            <el-button 
                    ...
                    style="float: right;"
                    @click="openAdd"
                    v-if="checkPermission(['manage excavators'])"
        ...
        <div v-if="this.showTable">
            <el-form :model="downtimesFilter" label-position="top">
                <el-row :gutter="20">
                    ...
                    <el-col :span="4">
                        <el-form-item label="Фильтр по экскаватору" label-position="top">
                            <el-select 
                                v-model="downtimesFilter.selectedExcavator" 
                                filterable
                                value-key="id_excavator"
                                :filter-method="excavatorFilter"
                            >
                                <el-option class="excavatorOption all"
                                    :key="null"
                                    label="Все"
                                    :value="null"
                                >
                                ...
                                <el-option class="excavatorOption"
                                    v-for="excavator in excavatorsListFiltered"
                                    :key="excavator.id_excavator"
                            ...
                            >
                                ...
                                >
                                    <div class="autocomplete-list-item">
                                            ...
                                            <br>
                                            <span class="garage-autocomplete">
                                                <b>Тип:</b>  {{ reason.type == 3 ? "Эксп.вне объёма" : (reason.type == 2 ? "Внеплановый": "Плановый") }}
                                            </span>
                                        </div>
                                </el-option>
                            </el-select>
        ...
        >
                ...
                <template slot-scope="props">
                    <div class="code-description" style="padding: 5px 15px;">
                        <el-row :gutter="20">
                            ...
                            <el-col :span="8">
                                <span><b>Тип простоя: &nbsp; </b>{{ props.row.type == 1 ? "Плановый" : (props.row.type == 2 ? "Внеплановый":"Эксп. вне объёма") }}</span>
                            </el-col>
                            <el-col :span="24">
                                <hr>
            ...
            <el-table-column :min-width="14" label="Экскаватор" prop="title_excavator" align="center"></el-table-column>
            ...
            <el-table-column :min-width="8"  label="Действия" align="center">
                <template slot-scope="scope">
                    <el-button v-if="!scope.row.end_time && checkPermission(['manage excavators'])"  type="info" icon="el-icon-edit" circle @click="openEdit(scope.row.id_downtime)"></el-button>
                    <el-button v-if="checkPermission(['manage excavators'])" type="danger" icon="el-icon-delete" circle @click="remove(scope.row.id_downtime)"></el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <AddDowntime  v-if="checkPermission(['manage excavators'])" :show="showAdd" :close="closeAdd" :placeId="id_place" :date="date"></AddDowntime>
        <EditDowntime  v-if="editingDowntimeId && checkPermission(['manage excavators'])" :show="showEdit" :close="closeEdit" :placeId="id_place" :date="date" :downtimeId="editingDowntimeId"></EditDowntime>
    </div>
</template>


<script>
import {getExcavatorDowntimesDailiy, deleteDowntime} from '@/api/excavators';
...
export default {
    ...
    methods: {
        checkPermission,
        async loadDowntimes() {
            const { data } = await getExcavatorDowntimesDailiy(this.date, this.id_place);
            ...
            const excavators = data.downtimes.map(d => ({
                id_excavator: d.id_excavator, 
                title: d.title_excavator,
                garage: d.garage,
            }));

            this.excavatorsList = excavators.filter((e,i,self) => i === self.findIndex((t) => t.id_excavator === e.id_excavator));
            this.excavatorsListFiltered = this.excavatorsList;
        ...
        paginate(p) 
        {
            if(this.downtimesFilter.selectedExcavator || this.downtimesFilter.selectedReason || this.downtimesFilter.search.trim()){
                const s = this.downtimesFilter.search.trim();
                let filtered = this.downtimesList;

                if(this.downtimesFilter.selectedExcavator)
                {
                    filtered = filtered.filter(r => r.id_excavator == this.downtimesFilter.selectedExcavator.id_excavator);
                }
                ...
                if(s)
                {
                    filtered = filtered.filter(r => (r.code.indexOf(s) != -1 || r.garage.indexOf(s) != -1 || r.title_excavator.indexOf(s) != -1 || r.reason.indexOf(s) != -1));
                }
        ...
        excavatorFilter(search)
        {
            this.excavatorsListFiltered =  this.excavatorsList.filter(r => (r.garage.indexOf(search) != -1 || r.title.indexOf(search) != -1 ));
        }, 
        reasonFilter(search)
        {
...
.el-select-dropdown__item.excavatorOption.all { 
