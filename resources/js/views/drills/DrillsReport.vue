<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="Буровые станки" prop="title"   align="center" min-width="160px">
                <template slot-scope="scope">
                    <div class="drillDowntimesClass" v-if="!scope.row.itSummary">
                        {{ scope.row.title }}
                    </div>
        ...
        <el-dialog title="Используемые коды в данном отчёте" width="60%" :visible.sync="showCodes" :before-close="closeCodes">
            ...
            >
                ...
                <el-table-column label="Для типа бурового станка" prop="title_technica_group" :min-width="16"></el-table-column>
            </el-table>
        </el-dialog>
...
<script>
import {drillDatasByReport} from "@/api/drills";
...
export default { 
    ...
    methods : {
        ...
        {
            const codesRes = await reasonsWithCodes(`/drill`);
        ...
        async loadData() 
        {
            const res = await drillDatasByReport(this.id_report, this.date);
        ...
        calculateDowntimes() 
        {
            ...
            // console.log(this.drillsData);
            const drills = this.drillsData.map((e, i, s) => {
                if(s.length - 1 != i) 
                {
                    return { 
                        title: e.title, 
            ...
            const linkItog = drills.at(-1).downtimes;
            ...
            const percentage = {
                ...
                downtimes: reportedReasonsList.reduce((acc, curr, i ) => {
                    acc[curr.id_reason] = { 
                        dayDowntime: Math.round(((linkItog[curr.id_reason].dayDowntime/this.drillsData.at(-1).downtimes.summary.dayDowntime)*100)*10)/10,
                        sinceDowntime: Math.round(((linkItog[curr.id_reason].sinceDowntime/this.drillsData.at(-1).downtimes.summary.sinceDowntime)*100)*10)/10,
                    }
                    return acc;
                }, {})
            }
            ...
            drills.push(percentage);
            this.drillDatasByReport = drills;
        },
        ...
        excelGetData() {
            ...
            this.drillDatasByReport.forEach((e, i, self) => {
                const currentDrill = [i+1, e.title, e.garage ?? ''];
                if(i >= self.length - 2)
                    currentDrill[0] = '';
                Object.keys(e.downtimes).forEach(key => {
                    currentDrill.push(e.downtimes[key].dayDowntime ? e.downtimes[key].dayDowntime : "0");
                    currentDrill.push(e.downtimes[key].sinceDowntime ? e.downtimes[key].sinceDowntime : "0");
                }); 
                currentDrill.push(e.workHour.day, e.workHour.since, e.kio.day, e.kio.since);
