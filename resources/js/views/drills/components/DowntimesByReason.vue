    ...
    >
        <div class="form-container">
            ...
            <el-select v-model="downtimesFilter.id_technica_group" @change="filterDowntimes('reset')">
                ...
                <el-option v-for="group in drillGroupTypes" :value="group.id_technica_group" :label="group.title_technica_type"></el-option>
            </el-select>


            <el-select 
                    v-model="downtimesFilter.id_drill" 
                    @change="filterDowntimes"      
                    filterable 
                >
                <el-option class="drillOption all"
                    label="Все буровые станки"
                    :value="null"
                >
                ...
                <el-option class="drillOption"
                    ...
                    :label="drill.title_drill"
                    :value="drill.id_drill"
                >
                    <div class="autocomplete-list-item">
                            <span class="name-autocomplete">
                                <b>Наименование:</b> {{ drill.title_drill }}
                            </span>
                            <br>
            ...
            >
                ...
                <el-table-column label="Тип бурового станка" prop="title_technica_type" :min-width="9" align="center"></el-table-column>
                <el-table-column label="Буровой станок" prop="title_drill" :min-width="15" align="center"></el-table-column>
...
<script>
// import {downtimesByReasonPlace} from "@/api/drills";
import {downtimesByReasonPlace} from "../../../api/drills";
...
export default {
    ...
    methods: { 
        async loadData() {
            ...
            const groupsDrills = data.downtimes.map(r=>({id_technica_group: r.id_technica_group, title_technica_type: r.title_technica_type}));
            this.drillGroupTypes = groupsDrills.reduce((acc, curr, ind) => {
            ...
            const drills = data.downtimes.map(r => ({id_drill: r.id_drill, title_drill: r.title_drill, title_technica_type: r.title_technica_type, id_technica_group: r.id_technica_group, garage: r.garage}));
            this.drillsList = drills.reduce((acc, curr, ind) => {
                if(!acc.find(a => a.id_drill === curr.id_drill)){
                    acc.push(curr);
                }
                return acc;
            }, []);
        ...
        filterDowntimes(reset) {
            ...
            if(this.downtimesFilter.id_technica_group)
            {
                ...
                drills = drills.filter(e => e.id_technica_group == this.downtimesFilter.id_technica_group);
            }

            if(reset == 'reset')
            {
                this.downtimesFilter.id_drill = null;
            }

            if(this.downtimesFilter.id_drill) 
            {
                downtimes = downtimes.filter(d => d.id_drill == this.downtimesFilter.id_drill);
            }
            
            this.drillsListFiltered = drills;
        ...
        drillFilter(search)
        {
            this.drillsListFiltered =  this.drillsList.filter(r => (r.garage.indexOf(search) != -1 || r.title_drill.indexOf(search) != -1 ));
        }, 
        ...
        excelHeader() 
        {
            ...
            const fourthRow = ['','','','','','','','', '']; 
            const fivthRow = ['№ п/п', 'Тип бурового станка', 'Буровой станок', 'Гараж №', 'Код', 'Описание кода простоя', 'Дата начала простоя', 'Дата окончания простоя', 'Длительность (в час)'];
