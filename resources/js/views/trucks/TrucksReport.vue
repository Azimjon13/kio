<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="Автосамосвалы" prop="title"   align="center" min-width="160px">
                <template slot-scope="scope">
                    <div class="truckDowntimesClass" v-if="!scope.row.itSummary">
                        {{ scope.row.title }}
                    </div>
                    <div v-else>{{ scope.row.title }}</div>
                </template>
            </el-table-column>
            <el-table-column label="Гараж №" prop="garage"  align="center" width="75"></el-table-column>
        ...
        <el-dialog title="Используемые коды в данном отчёте" width="60%" :visible.sync="showCodes" :before-close="closeCodes">
            ...
            >
                ...
                <el-table-column label="Для типа автосамосвала" prop="title_technica_group" :min-width="16"></el-table-column>
            </el-table>
        </el-dialog>
...
<script>
import {truckDatasByReport} from "@/api/trucks";
...
export default { 
    ...
    methods : {
        ...
        {
            const codesRes = await reasonsWithCodes(`/truck`);
        ...
        async loadData() 
        {
            const res = await truckDatasByReport(this.id_report, this.date);
            ...
            this.trucksData = res.data.plans_facts.map(e => ({
                ...
                downtimes: this.reasonsList.reduce((acc, cur, i) => {
                    // сначала ищем по Автосамосвалы и По текущему простою есть ли данные
                    /////
        ...
        calculateDowntimes() 
        {
            ...
            const trucks = this.trucksData.map((e, i, s) => {
                if(s.length - 1 != i) 
                {
                    return { 
                        title: e.title, 
            ...
            const linkItog = trucks.at(-1).downtimes;
            ...
            const percentage = {
                ...
                downtimes: reportedReasonsList.reduce((acc, curr, i ) => {
                    acc[curr.id_reason] = { 
                        dayDowntime: Math.round(((linkItog[curr.id_reason].dayDowntime/this.trucksData.at(-1).downtimes.summary.dayDowntime)*100)*10)/10,
                        sinceDowntime: Math.round(((linkItog[curr.id_reason].sinceDowntime/this.trucksData.at(-1).downtimes.summary.sinceDowntime)*100)*10)/10,
                    }
                    return acc;
                }, {})
            }
            ...
            trucks.push(percentage);
            this.truckDatasByReport = trucks;
        },
        ...
        excelGetData() {
            ...
            this.truckDatasByReport.forEach((e, i, self) => {
                const currentTruck = [i+1, e.title, e.garage ?? ''];
                if(i >= self.length - 2)
                    currentTruck[0] = '';
                Object.keys(e.downtimes).forEach(key => {
                    currentTruck.push(e.downtimes[key].dayDowntime ? e.downtimes[key].dayDowntime : "0");
                    currentTruck.push(e.downtimes[key].sinceDowntime ? e.downtimes[key].sinceDowntime : "0");
                }); 
                currentTruck.push(e.workHour.day, e.workHour.since, e.kio.day, e.kio.since);
