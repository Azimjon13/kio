<template>
    <div class="app-container">
        <h3>Остановки автосамосвалов за сегодня {{ showTable ? "": "(скрыто)" }}
            ...
            <el-button 
                    ...
                    style="float: right;"
                    @click="openAdd"
                    v-if="checkPermission(['manage trucks'])"
        ...
        <div v-if="this.showTable">
            <el-form :model="downtimesFilter" label-position="top">
                <el-row :gutter="20">
                    ...
                    <el-col :span="4">
                        <el-form-item label="Фильтр по автосамосвалам" label-position="top">
                            <el-select 
                                v-model="downtimesFilter.selectedTruck" 
                                filterable
                                value-key="id_truck"
                                :filter-method="truckFilter"
                            >
        ...
        >
            ...
            <el-table-column :min-width="14" label="Автосамосвал" prop="title_truck" align="center"></el-table-column>
            ...
            <el-table-column :min-width="8"  label="Действия" align="center">
                <template slot-scope="scope">
                    <el-button v-if="!scope.row.end_time && checkPermission(['manage trucks'])"  type="info" icon="el-icon-edit" circle @click="openEdit(scope.row.id_downtime)"></el-button>
                    <el-button v-if="checkPermission(['manage trucks'])" type="danger" icon="el-icon-delete" circle @click="remove(scope.row.id_downtime)"></el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <AddDowntime  v-if="checkPermission(['manage trucks'])" :show="showAdd" :close="closeAdd" :placeId="id_place" :date="date"></AddDowntime>
        <EditDowntime  v-if="editingDowntimeId && checkPermission(['manage trucks'])" :show="showEdit" :close="closeEdit" :placeId="id_place" :date="date" :downtimeId="editingDowntimeId"></EditDowntime>
    </div>
</template>


<script>
import {getTruckDowntimesDailiy, deleteDowntime} from '@/api/trucks';
...
export default {
    ...
    methods: {
        checkPermission,
        async loadDowntimes() {
            const { data } = await getTruckDowntimesDailiy(this.date, this.id_place);
            ...
            const trucks = data.downtimes.map(d => ({
                id_truck: d.id_truck, 
                title: d.title_truck,
                garage: d.garage,
            }));

            this.trucksList = trucks.filter((e,i,self) => i === self.findIndex((t) => t.id_truck === e.id_truck));
            this.trucksListFiltered = this.trucksList;
        ...
        paginate(p) 
        {
            if(this.downtimesFilter.selectedTruck || this.downtimesFilter.selectedReason || this.downtimesFilter.search.trim()){
                const s = this.downtimesFilter.search.trim();
                let filtered = this.downtimesList;

                if(this.downtimesFilter.selectedTruck)
                {
                    filtered = filtered.filter(r => r.id_truck == this.downtimesFilter.selectedTruck.id_truck);
                }
                ...
                if(s)
                {
                    filtered = filtered.filter(r => (r.code.indexOf(s) != -1 || r.garage.indexOf(s) != -1 || r.title_truck.indexOf(s) != -1 || r.reason.indexOf(s) != -1));
                }
        ...
        truckFilter(search)
        {
            this.trucksListFiltered =  this.trucksList.filter(r => (r.garage.indexOf(search) != -1 || r.title.indexOf(search) != -1 ));
        }, 
        reasonFilter(search)
        {
