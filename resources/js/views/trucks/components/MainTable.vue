<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="Автосамосвалы" prop="title"  fixed align="center" min-width="160px">
                <template slot-scope="scope">
                    <div class="truckDowntimesClass" v-if="!scope.row.itSummary" @click="handleShowTruckDowntimesDialog(scope.row.id_place_truck)">
                        {{ scope.row.title }}
                    </div>
        ...
        <TruckDowntimes 
            v-if="currentSelectedIdPlaceTruck" 
            :show="showTruckDowntimesDialog" 
            :close="handleCloseTruckDowntimesDialog"
            :id_place_truck="currentSelectedIdPlaceTruck"
            :date="date"
        ></TruckDowntimes>
        <DowntimesByReason
        ...
        ></DowntimesByReason>
        <PlaceReportsList :routerLink="'trucksReport'" :show="showReportsList" :close="closeReportsList" :id_technica_type = "1" :id_place="id_place" :date="date"></PlaceReportsList>
...
<script>
import {getMainData, storePlan, storeFact} from "@/api/trucks";
import {getPlaceForExcel} from "@/api/places";
import checkPermission from '@/utils/permission'; // Permission checking
import TruckDowntimes from "./TruckDowntimes.vue";
...
export default { 
    components: { TruckDowntimes, DowntimesByReason, PlaceReportsList, SvodTable },
    props: {
        id_place: Number,
        date: String,
        updater: Number,
    ...
    data() {
        return {
            ...
            // Простои по одному конкретному автосамосвалу
            currentSelectedIdPlaceTruck: null, 
            showTruckDowntimesDialog: false,
            ...
            trucksData: [], // Главный список по причинам
            reasonsColumns: {}, 
    ...
    methods: {
        ...
        async loadData() 
        {
            ...
            /*
                res === {
                    plans_facts: [], <-- массив объектов по каждому автосамосвалу и его плану и факту
                    downtimes: []  <-- массив объектов с причинами 
                                        могут быть дубликаты т.к. по разным автосамосвалам могут встречаться
            ...
            */
            ...
            this.trucksData = res.data.plans_facts.map(e => ({
                ...e,  // тут получаем всё остальное (план и факт, данные автосамосвала название гаражный и т.д.)
                // далее мы составляем для данного эксаватора часы простоев по спискам простоев
                downtimes: this.reasonsList.reduce((acc, cur, i) => {
                    // сначала ищем по Автосамосвалам и По текущему простою есть ли данные
                    let a = res.data.downtimes.find(d => (
                        d.id_place_truck == e.id_place_truck && 
                        d.id_truck == e.id_truck && 
                    ...
                    // иначе если данных нет, то мы записываем пустой Простой без часов простоев
                        ...
                        acc[cur.id_reason]= {
                            ...cur,
                            id_place_truck: e.id_place_truck, 
                            id_truck: e.id_truck, 
                            dayDowntime: null, 
                            sinceDowntime: null, 
                        };
            ...
            this.trucksData = this.trucksData.map(e => {
                ...
                // в цикле получаем все причины простоев  для текущего автосамосвала
                ...
                const hours = new Date(this.date).getDate() * 24; // Вычисляем сколько должен был работать 1 автосамосвал с начала месяца
            ...
            const dayHours = 24*this.trucksData.length;
        ...
        changeDowntimeView() 
        {
            ...
            this.$nextTick(() => { this.$refs['trucksDatas'].doLayout(); })

        }, 

        handleShowTruckDowntimesDialog(id_place_truck) 
        {
            console.log(id_place_truck);
            this.currentSelectedIdPlaceTruck = id_place_truck;
            this.showTruckDowntimesDialog = true;
        },

        handleCloseTruckDowntimesDialog()
        {
            this.showTruckDowntimesDialog = false;
            this.currentSelectedIdPlaceTruck = null;
        },

        handleShowReasonDowntimesDialog(reason)
        {
            const day = this.trucksData.at(-1).downtimes[reason.key].dayDowntime;
            const since = this.trucksData.at(-1).downtimes[reason.key].sinceDowntime;
        ...
        async excelHeader() 
        {
            ...
            const merges = [];
            const multiHeader = [];
            const firstRow = ['№ п/п', 'Автосамосвалы', 'Гараж №'];
            const secondRow = ['','',''];
            const thirdRow = ['','',''];
            const fourthRow = ['','',''];
            ...
            let title = `Оперативные производственные показатели автосамосвалов `;
        ...
        excelGetData() {
            const data = [];
            this.trucksData.forEach((e, i, self ) => {

                const currentTruck = [i+1, e.title, e.garage ?? ''];
                if(i == self.length - 1) 
                    currentTruck[0] = '';
                ...
                if(!this.excelConfig.downtimesShort)
                {
                    ...
                    planned.forEach(r => { 
                        if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1)  
                            currentTruck.push(e.downtimes[r].dayDowntime);
                        if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                            currentTruck.push(e.downtimes[r].sinceDowntime);
                    });
                    ...
                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1)
                        currentTruck.push(e.downtimes['summary'].dayDowntime);
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1)
                        currentTruck.push(e.downtimes['summary'].sinceDowntime);

                }
                else 
                {
                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                        currentTruck.push(e.downtimes['planned'].dayDowntime);
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1)
                        currentTruck.push(e.downtimes['planned'].sinceDowntime);

                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                        currentTruck.push(e.downtimes['unscheduled'].dayDowntime);
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1)
                        currentTruck.push(e.downtimes['unscheduled'].sinceDowntime);
                ...
                if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                    currentTruck.push(e.workHour.day);
                if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                    currentTruck.push(e.workHour.since);
                if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                    currentTruck.push(e.kio.day);
                if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                    currentTruck.push(e.kio.since);

                data.push(currentTruck);
            });
            return data;
        },
