<template>
    <el-dialog 
        title="Данные простоев по автосамосвалу с начала месяца" 
        width="70%" 
    ...
    >
        <div class="form-container">
            <el-descriptions direction="vertical" :column="4" border>
                ...
                <el-descriptions-item label="Автосамосвал">
                    {{ place.truck }}
                </el-descriptions-item>
...
<script>
import { truckDowntimes } from '@/api/trucks';

export default  {
    props: {
        show: Boolean,
        close: Function, 
        id_place_truck: Number,
        date: String,
    },
    data() {
        return {
    ...
    created() {
        this.loadDowntimesTruck();
    },
    methods:{
        async loadDowntimesTruck() {
            const {data} = await truckDowntimes({id_place_truck: this.id_place_truck, date: this.date});
        ...
        excelHeader() 
        {
            ...
            const firstRow = ['', 'Площадка', '', 'Автосамосвал', '', 'Гараж №', 'На дату', ''];
            const secondRow = ['', this.place.place, '', this.place.truck, '', this.place.garage, 'С: '+new Date(this.shifts.shift_month_start).toLocaleString(undefined, {year:'numeric', month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit'}).replace(',', ' '), ''];
            ...
            const merges = ['B1:C1',  // Первая строка Площадка на 2 колонки
                            ...
                            'D2:E3',  // Вторая строка Автосамосвал на 2 строки и на 2 колонки
