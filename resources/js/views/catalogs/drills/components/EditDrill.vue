<template>
    <el-dialog title="Редактирование бурового станка" width="70%" :visible.sync="this.show" :before-close="close">
        <div class="form-container">
            <el-form ref="editDrillForm" :model="drill" label-position="top" >
                <el-row :gutter="20">
                    <el-col :span="10">
                    ...
                    <el-col :span="7">
                        <el-form-item label="Тип бурового станка">
                            <el-select v-model="drill.id_group" disabled>
                                <el-option 
                    ...
                    <el-col :span="7">
                        <el-form-item label="Статус">
                            <el-select v-model="drill.status">
                            ...
                            <el-date-picker 
                                ...
                                :pickerOptions = "{
                                    disabledDate(time) {
                                        ...
                                        const disDate = drill.drill_places[ind].date_to;
                    ...
                    <el-table-column :min-width="8" align="center">
                        <template slot-scope="scope">
                            ...
                            <el-button  
                                type="danger" 
                                icon="el-icon-delete" 
                                @click="removeDrillPlace(scope.row)"
                                circle
                            ></el-button>
                        </template>
...
<script>
import {getDrill, updateDrill, deleteDrillPlace, getDrillGroups} from "@/api/drills";
...
export default { 
    ...
    computed: {
        showAddBtn: function() {
            return this.drill.drill_places.filter(p=>p.flag).length == 0 && 
                   this.drill.drill_places.filter(p=> !(p.id_manager || p.id_parent || p.id_place || p.date_to)).length == 0;
            }
    },
    methods: {
        async loadTypes() {
        ...
        async loadDrill() {
            const {data} = await getDrill(this.drillId);

            this.drill = {
                ...data, 
                drill_places:  data.drill_places.map(p => {
                    if(p.date_to) 
        ...
        async saveDrill() {
            const res = await updateDrill(this.drillId, this.drill);
            console.log(res);
        ...
        addDrillPlace()
        {
            this.drill.drill_places.push({
                garage: "",
                date_from: "",
                date_to: "",
                id_drill: this.drill.id, 
                id_place: null,
                id_parent: null,
                id_manager: null,
                flag: true,
        ...
        async removeDrillPlace(row) 
        {
            if(row.id) 
            {
                ...
                const res = await deleteDrillPlace(row.id);
            ...
            const ind = this.drill.drill_places.indexOf(row);
            this.drill.drill_places.splice(ind, 1);
        } 
    }
