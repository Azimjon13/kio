<template>
    <el-dialog title="Редактирование ЦПТ" width="70%" :visible.sync="this.show" :before-close="close">
        <div class="form-container">
            <el-form ref="editCptForm" :model="cpt" label-position="top" >
                <el-row :gutter="20">
                    <el-col :span="10">
                    ...
                    <el-col :span="7">
                        <el-form-item label="Тип ЦПТ">
                            <el-select v-model="cpt.id_group" disabled>
                                <el-option 
                            ...
                            <el-date-picker 
                                ...
                                :pickerOptions = "{
                                    disabledDate(time) {
                                        ...
                                        const disDate = cpt.cpt_places[ind].date_to;
                    ...
                    <el-table-column :min-width="8" align="center">
                        <template slot-scope="scope">
                            ...
                            <el-button  
                                type="danger" 
                                icon="el-icon-delete" 
                                @click="removeCptPlace(scope.row)"
                                circle
                            ></el-button>
                        </template>
...
<script>
import {getCpt, updateCpt, deleteCptPlace, getCptGroups} from "@/api/cpts";
...
export default { 
    ...
    computed: {
        showAddBtn: function() {
            return this.cpt.cpt_places.filter(p=>p.flag).length == 0 && 
                   this.cpt.cpt_places.filter(p=> !(p.id_manager || p.id_parent || p.id_place || p.date_to)).length == 0;
            }
    },
    methods: {
        async loadTypes() {
        ...
        async loadCpt() {
            const {data} = await getCpt(this.cptId);

            this.cpt = {
                ...data, 
                cpt_places:  data.cpt_places.map(p => {
                    if(p.date_to) 
        ...
        async saveCpt() {
            const res = await updateCpt(this.cptId, this.cpt);
            console.log(res);
            if(res.status && res.errors == null)
            {
                this.$message({
                    message: 'ЦПТ успешно обновлен!',
                    type: 'success',
                    duration: 5 * 1000,
                });
                this.close();
        ...
        async removeCptPlace(row) 
        {
            if(row.id) 
            {
                ...
                const res = await deleteCptPlace(row.id);
                ...
                this.$message({
                    message:`Площадка дислокации ЦПТ успешно удалена!`, 
                    type: "success",
                    duration: 5*1000,
                });
            }
            const ind = this.cpt.cpt_places.indexOf(row);
            this.cpt.cpt_places.splice(ind, 1);
        } 
    }
