<template>
    <el-dialog title="Добавление Кода простоя" width="40%" :visible.sync="this.show" :before-close="this.close">
...
<script>
import { storeCode } from '@/api/codes';
import { technicaGroups, technicaTypes } from '@/api/technica';
import { getReasonsSelect } from "@/api/reasons";
...
export default {
   ...
   methods: {
        ...
        async saveCode()
        {
            const res = await storeCode(this.code);
