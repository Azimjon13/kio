<template>
    <el-dialog title="Редактирование Отчёта" width="60%" :visible.sync="this.show" :before-close="this.close">
        <div class="form-container">
            ...
            <span slot="footer" class="dialog-footer">
                <el-button type="primary" @click="save" v-if="checkPermission(['manage catalog-reports'])">
                    Сохранить
                </el-button>
...
export default  {
    ...
    methods: {
        ...
        async loadReport() {
            ...
            const {data} = await getReport(this.id_report);

            this.report = data;
            
            await this.afterSelectManagerOrTechnicaType("FIRST");
            await this.afterSelectRudnik("FIRST");
        ...
        async afterSelectManagerOrTechnicaType(firstCall)
        {
            if(this.report.id_technica_type && this.report.id_manager)
            {
                ...
                if(firstCall == "FIRST") 
                {
                    const downtimesCodes = this.report.downtime_codes.map(r => r.id_code);
                    this.reasons = res.data.map(r => ({
                        ...r, 
                        codes: r.codes.map(c => ({...c, selected: downtimesCodes.includes(c.id)}))
                    }))                    
                }
                else {
                ...
                console.log(this.rudniks);
                
            }
        }, 
        async afterSelectRudnik(firstCall) 
        {
            this.report.id_place = firstCall == "FIRST" ? this.report.id_place : null;
        ...
        async save() 
        {
            ...
            const res = await updateReport(this.id_report, request);
