<template>
    <el-dialog title="Добавление простоя Тепловоза" width="50%" :visible.sync="this.show" :before-close="close">
        <div class="form-container">
            <el-form ref="addDowntimeLocomotiveForm" :model="downtime" label-position="top" >
                <el-descriptions :column="1" border :label-style="{'min-width': '270px'}">
                    <el-descriptions-item >
                        <template slot="label">
                           Выберите тип Тепловоза
                        </template>
                        <el-select v-model="downtime.technica_group"   :disabled="true">
                            <el-option 
                                v-for="g in locomotiveGroups" 
                                :value="g.id" 
                                :label="g.name"
                            ></el-option>
                        </el-select>
                    ...
                    <el-descriptions-item>
                        ...
                        <el-select
                            :disabled="true"
                            v-model="downtime.locomotive"
                            placeholder="Введите для поиска Тепловоза"
                            filterable
                            remote
                            reserve-keyword
                            :remote-method="searchLocomotiveSelect"
                            :loading="loadingLocomotive"
                            value-key="id"
                        >
                            <el-option class="locomotiveOption" 
                                    v-for="locomotive in locomotivesList" 
                                    :key="locomotive.id" 
...
<script>
import {getLocomotiveGroups, searchLocomotive, updateDowntime, getDowntime} from "@/api/locomotives";
...
export default { 
    ...
    methods: {
        async loadDowntime() 
        {
            ...
            this.locomotivesList = [data.place_locomotive]; 
            ...
            this.downtime = {
                ...
                locomotive: data.place_locomotive,
            };
        },
        ...
        {
            let dateFrom = new Date(this.downtime.locomotive.date_from).getTime() - 86400*1000;
            let dateTo = new Date(this.downtime.locomotive.date_to).getTime();
        ...
        async searchLocomotiveSelect(search)
        {
            if(search.trim() !== '' && search.length >= 2) 
            {
                this.loadingLocomotive = true;
                const query = {
                ...
                let {data} = await searchLocomotive(query);
                this.loadingLocomotive = false;
                if(data != null)
                {
                    this.locomotivesList = data;
                }
            }
            else
            {
                this.locomotivesList = [];
            }
        },
        ...
        async saveDowntime()
        {
            const newDowntime = {
                id_place: this.downtime.id_place, 
                id_locomotive: this.downtime.id_locomotive, 
                id_place_locomotive: this.downtime.id_place_locomotive, 
            ...
            {
                this.$message({
                    message: 'Простой Тепловоза успешно добавлен!',
                    type: 'success',
                    duration: 5 * 1000,
                });
                this.close();
