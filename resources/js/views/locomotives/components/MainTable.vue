<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="Тепловоз" prop="title"  fixed align="center" min-width="160px">
                <template slot-scope="scope">
                    <div class="locomotiveDowntimesClass" v-if="!scope.row.itSummary" @click="handleShowLocomotiveDowntimesDialog(scope.row.id_place_locomotive)">
                        {{ scope.row.title }}
                    </div>
                    <div v-else>{{ scope.row.title }}</div>
                </template>
            </el-table-column>
            <el-table-column label="Серийный №" prop="serial_number"   fixed align="center" width="75"></el-table-column>
        ...
        <LocomotiveDowntimes 
            v-if="currentSelectedIdPlaceLocomotive" 
            :show="showLocomotiveDowntimesDialog" 
            :close="handleCloseLocomotiveDowntimesDialog"
            :id_place_locomotive="currentSelectedIdPlaceLocomotive"
            :date="date"
        ></LocomotiveDowntimes>
        <DowntimesByReason
        ...
        ></DowntimesByReason>
        <PlaceReportsList :routerLink="'locomotivesReport'" :show="showReportsList" :close="closeReportsList" :id_technica_type = "1" :id_place="id_place" :date="date"></PlaceReportsList>
...
<script>
import {getMainData, storePlan, storeFact} from "@/api/locomotives";
import {getPlaceForExcel} from "@/api/places";
import checkPermission from '@/utils/permission'; // Permission checking
import LocomotiveDowntimes from "./LocomotiveDowntimes.vue";
...
export default { 
    components: { LocomotiveDowntimes, DowntimesByReason, PlaceReportsList, SvodTable },
    props: {
        id_place: Number,
        date: String,
        updater: Number,
    ...
    computed: {
        ...
        {
            return this.locomotivesData.at(-1);
        }
    },
    mounted() {
        this.loadData();
    ...
    data() {
        return {
            ...
            // Простои по одному конкретному Тепловозу
            currentSelectedIdPlaceLocomotive: null, 
            showLocomotiveDowntimesDialog: false,
    ...
    methods: {
        ...
        async loadData() 
        {
            ...
            /*
                res === {
                    plans_facts: [], <-- массив объектов по каждому Тепловозу и его плану и факту
                    downtimes: []  <-- массив объектов с причинами 
                                        могут быть дубликаты т.к. по разным Тепловозам могут встречаться
            ...
            */
            ...
            this.locomotivesData = res.data.plans_facts.map(e => ({
                ...e,  // тут получаем всё остальное (план и факт, данные Тепловозу название гаражный и т.д.)
                // далее мы составляем для данного эксаватора часы простоев по спискам простоев
                downtimes: this.reasonsList.reduce((acc, cur, i) => {
                    // сначала ищем по Тепловозу и По текущему простою есть ли данные
            ...
            this.locomotivesData = this.locomotivesData.map(e => {
                ...
                const hours = new Date(this.date).getDate() * 24; // Вычисляем сколько должен был работать 1 Тепловоз с начала месяца
        ...
        changeDowntimeView() 
        {
            ...
            this.$nextTick(() => { this.$refs['locomotivesDatas'].doLayout(); })

        }, 

        handleShowLocomotiveDowntimesDialog(id_place_locomotive) 
        {
            console.log(id_place_locomotive);
            this.currentSelectedIdPlaceLocomotive = id_place_locomotive;
            this.showLocomotiveDowntimesDialog = true;
        },

        handleCloseLocomotiveDowntimesDialog()
        {
            this.showLocomotiveDowntimesDialog = false;
            this.currentSelectedIdPlaceLocomotive = null;
        },

        handleShowReasonDowntimesDialog(reason)
        {
            const day = this.locomotivesData.at(-1).downtimes[reason.key].dayDowntime;
            const since = this.locomotivesData.at(-1).downtimes[reason.key].sinceDowntime;
        ...
        async excelHeader() 
        {
            ...
            const merges = [];
            const multiHeader = [];
            const firstRow = ['№ п/п', 'Тепловоз', 'Гараж №'];
            const secondRow = ['','',''];
            const thirdRow = ['','',''];
            const fourthRow = ['','',''];
            ...
            let title = `Оперативные производственные показатели Тепловозу `;
        ...
        excelGetData() {
            const data = [];
            this.locomotivesData.forEach((e, i, self ) => {
                ...
                if(!this.excelConfig.downtimesShort)
                {
                    ...
                    planned.forEach(r => { 
                        if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1)  
                            currentLocomotive.push(e.downtimes[r].dayDowntime);
                        if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                            currentLocomotive.push(e.downtimes[r].sinceDowntime);
                    });
                    ...
                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1)
                        currentLocomotive.push(e.downtimes['summary'].dayDowntime);
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1)
                        currentLocomotive.push(e.downtimes['summary'].sinceDowntime);

                }
                else 
                {
                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                        currentLocomotive.push(e.downtimes['planned'].dayDowntime);
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1)
                        currentLocomotive.push(e.downtimes['planned'].sinceDowntime);

                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                        currentLocomotive.push(e.downtimes['unscheduled'].dayDowntime);
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1)
                        currentLocomotive.push(e.downtimes['unscheduled'].sinceDowntime);
                ...
                if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                    currentLocomotive.push(e.workHour.day);
                if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                    currentLocomotive.push(e.workHour.since);
                if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                    currentLocomotive.push(e.kio.day);
                if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                    currentLocomotive.push(e.kio.since);
...
.locomotiveDowntimesClass:hover { 
    cursor: pointer;
