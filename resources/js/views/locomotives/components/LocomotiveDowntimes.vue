<template>
    <el-dialog 
        title="Данные простоев по Тепловозам с начала месяца" 
        width="70%" 
    ...
    >
        <div class="form-container">
            <el-descriptions direction="vertical" :column="4" border>
                ...
                <el-descriptions-item label="Тепловоз">
                    {{ place.locomotive }}
                </el-descriptions-item>
                <el-descriptions-item label="Серийный №">
                    {{ place.serial_number }}
                </el-descriptions-item>
            ...
            <el-table
                ...
                ref="locomotiveDowntimes"
                class="locomotiveDowntimesTable"
...
<script>
import { locomotiveDowntimes } from '@/api/locomotives';

export default  {
    props: {
        show: Boolean,
        close: Function, 
        id_place_locomotive: Number,
        date: String,
    },
    data() {
        return {
    ...
    created() {
        this.loadDowntimesLocomotive();
    },
    methods:{
        async loadDowntimesLocomotive() {
            const {data} = await locomotiveDowntimes({id_place_locomotive: this.id_place_locomotive, date: this.date});
        ...
        excelHeader() 
        {
            ...
            const firstRow = ['', 'Площадка', '', 'Тепловоз', '', 'Серийный №', 'На дату', ''];
            const secondRow = ['', this.place.place, '', this.place.locomotive, '', this.place.serial_number, 'С: '+new Date(this.shifts.shift_month_start).toLocaleString(undefined, {year:'numeric', month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit'}).replace(',', ' '), ''];
            ...
            const merges = ['B1:C1',  // Первая строка Площадка на 2 колонки
                            ...
                            'D2:E3',  // Вторая строка Тепловоз на 2 строки и на 2 колонки
