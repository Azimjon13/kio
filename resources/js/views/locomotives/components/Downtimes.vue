<template>
    <div class="app-container">
        <h3>Остановки Тепловозов за сегодня {{ showTable ? "": "(скрыто)" }}
            ...
            <el-button 
                    ...
                    style="float: right;"
                    @click="openAdd"
                    v-if="checkPermission(['manage locomotives'])"
        ...
        <div v-if="this.showTable">
            <el-form :model="downtimesFilter" label-position="top">
                <el-row :gutter="20">
                    ...
                    <el-col :span="4">
                        <el-form-item label="Фильтр по Тепловозу" label-position="top">
                            <el-select 
                                v-model="downtimesFilter.selectedLocomotive" 
                                filterable
                                value-key="id_locomotive"
                                :filter-method="locomotiveFilter"
                            >
                                ...
                                <el-option class="locomotiveOption"
                                    ...
                                    :key="locomotive.id_locomotive"
                                    :label="locomotive.title"
                                    :value="locomotive"
                                >
                                    <div class="autocomplete-list-item">
                                            <span class="name-autocomplete">
                                                <b>Наименование:</b> {{ locomotive.title }}
                                            </span>
                                            <br>
        ...
        >
            ...
            <el-table-column :min-width="14" label="Тепловоз" prop="title_locomotive" align="center"></el-table-column>
            <el-table-column :min-width="7"  label="Серийный №" prop="serial_number" align="center"></el-table-column>
            ...
            <el-table-column :min-width="8"  label="Действия" align="center">
                <template slot-scope="scope">
                    <el-button v-if="!scope.row.end_time && checkPermission(['manage locomotives'])"  type="info" icon="el-icon-edit" circle @click="openEdit(scope.row.id_downtime)"></el-button>
                    <el-button v-if="checkPermission(['manage locomotives'])" type="danger" icon="el-icon-delete" circle @click="remove(scope.row.id_downtime)"></el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <AddDowntime  v-if="checkPermission(['manage locomotives'])" :show="showAdd" :close="closeAdd" :placeId="id_place" :date="date"></AddDowntime>
        <EditDowntime  v-if="editingDowntimeId && checkPermission(['manage locomotives'])" :show="showEdit" :close="closeEdit" :placeId="id_place" :date="date" :downtimeId="editingDowntimeId"></EditDowntime>
    </div>
</template>


<script>
import {getLocomotiveDowntimesDailiy, deleteDowntime} from '@/api/locomotives';
...
export default {
    ...
    methods: {
        checkPermission,
        async loadDowntimes() {
            const { data } = await getLocomotiveDowntimesDailiy(this.date, this.id_place);
            ...
            const locomotives = data.downtimes.map(d => ({
                id_locomotive: d.id_locomotive, 
                title: d.title_locomotive,
                serial_number: d.serial_number,
            }));

            this.locomotivesList = locomotives.filter((e,i,self) => i === self.findIndex((t) => t.id_locomotive === e.id_locomotive));
            this.locomotivesListFiltered = this.locomotivesList;
        ...
        paginate(p) 
        {
            if(this.downtimesFilter.selectedLocomotive || this.downtimesFilter.selectedReason || this.downtimesFilter.search.trim()){
                const s = this.downtimesFilter.search.trim();
                let filtered = this.downtimesList;

                if(this.downtimesFilter.selectedLocomotive)
                {
                    filtered = filtered.filter(r => r.id_locomotive == this.downtimesFilter.selectedLocomotive.id_locomotive);
                }
                ...
                if(s)
                {
                    filtered = filtered.filter(r => (r.code.indexOf(s) != -1 || r.serial_number.indexOf(s) != -1 || r.title_locomotive.indexOf(s) != -1 || r.reason.indexOf(s) != -1));
                }
        ...
        {
            this.locomotivesListFiltered =  this.locomotivesList.filter(r => (r.serial_number.indexOf(search) != -1 || r.title.indexOf(search) != -1 ));
        }, 
        reasonFilter(search)
        {
