<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="ЦПТ" prop="title"   align="center" min-width="160px">
                <template slot-scope="scope">
                    <div class="cptDowntimesClass" v-if="!scope.row.itSummary">
                        {{ scope.row.title }}
                    </div>
        ...
        <el-dialog title="Используемые коды в данном отчёте" width="60%" :visible.sync="showCodes" :before-close="closeCodes">
            ...
            >
                ...
                <el-table-column label="Для типа ЦПТ" prop="title_technica_group" :min-width="16"></el-table-column>
            </el-table>
        </el-dialog>
...
<script>
import {cptDatasByReport} from "@/api/cpts";
...
export default { 
    ...
    methods : {
        ...
        {
            const codesRes = await reasonsWithCodes(`/cpt`);
        ...
        async loadData() 
        {
            const res = await cptDatasByReport(this.id_report, this.date);
            ...
            this.cptsData = res.data.plans_facts.map(e => ({
                ...
                ...e,  // тут получаем всё остальное (план и факт, данные ЦПТ название гаражный и т.д.)

                // далее мы составляем для данного эксаватора часы простоев по спискам простоев
                downtimes: this.reasonsList.reduce((acc, cur, i) => {
                    // сначала ищем по ЦПТ и По текущему простою есть ли данные
                    /////
            ...
            this.cptsData = this.cptsData.map(e => {
                ...
                // в цикле получаем все причины простоев  для текущего ЦПТ
        ...
        calculateDowntimes() 
        {
            ...
            // console.log(this.cptsData);
            const cpts = this.cptsData.map((e, i, s) => {
                if(s.length - 1 != i) 
                {
                    return { 
                        title: e.title, 
            ...
            const linkItog = cpts.at(-1).downtimes;
            ...
            const percentage = {
                ...
                downtimes: reportedReasonsList.reduce((acc, curr, i ) => {
                    acc[curr.id_reason] = { 
                        dayDowntime: Math.round(((linkItog[curr.id_reason].dayDowntime/this.cptsData.at(-1).downtimes.summary.dayDowntime)*100)*10)/10,
                        sinceDowntime: Math.round(((linkItog[curr.id_reason].sinceDowntime/this.cptsData.at(-1).downtimes.summary.sinceDowntime)*100)*10)/10,
                    }
                    return acc;
                }, {})
            }
            ...
            cpts.push(percentage);
            this.cptDatasByReport = cpts;
        },
        ...
        excelGetData() {
            ...
            this.cptDatasByReport.forEach((e, i, self) => {
                const currentCpt = [i+1, e.title, e.garage ?? ''];
                if(i >= self.length - 2)
                    currentCpt[0] = '';
                Object.keys(e.downtimes).forEach(key => {
                    currentCpt.push(e.downtimes[key].dayDowntime ? e.downtimes[key].dayDowntime : "0");
                    currentCpt.push(e.downtimes[key].sinceDowntime ? e.downtimes[key].sinceDowntime : "0");
                }); 
                currentCpt.push(e.workHour.day, e.workHour.since, e.kio.day, e.kio.since);
