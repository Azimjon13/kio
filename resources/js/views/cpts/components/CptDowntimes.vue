<template>
    <el-dialog 
        title="Данные простоев по ЦПТ с начала месяца" 
        width="70%" 
    ...
    >
        <div class="form-container">
            <el-descriptions direction="vertical" :column="4" border>
                ...
                <el-descriptions-item label="ЦПТ">
                    {{ place.cpt }}
                </el-descriptions-item>
...
<script>
import { cptDowntimes } from '@/api/cpts';
...
export default  {
    ...
    created() {
        this.loadDowntimesCpt();
    },
    methods:{
        async loadDowntimesCpt() {
            const {data} = await cptDowntimes({id_place_cpt: this.id_place_cpt, date: this.date});
        ...
        excelHeader() 
        {
            ...
            const firstRow = ['', 'Площадка', '', 'ЦПТ', '', 'Гараж №', 'На дату', ''];
            const secondRow = ['', this.place.place, '', this.place.cpt, '', this.place.garage, 'С: '+new Date(this.shifts.shift_month_start).toLocaleString(undefined, {year:'numeric', month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit'}).replace(',', ' '), ''];
            ...
            const merges = ['B1:C1',  // Первая строка Площадка на 2 колонки
                            ...
                            'D2:E3',  // Вторая строка ЦПТ на 2 строки и на 2 колонки
