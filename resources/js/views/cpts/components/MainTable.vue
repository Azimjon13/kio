<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="ЦПТ" prop="title"  fixed align="center" min-width="160px">
                <template slot-scope="scope">
                    <div class="cptDowntimesClass" v-if="!scope.row.itSummary" @click="handleShowCptDowntimesDialog(scope.row.id_place_cpt)">
                        {{ scope.row.title }}
                    </div>
        ...
        <CptDowntimes 
            v-if="currentSelectedIdPlaceCpt" 
            :show="showCptDowntimesDialog" 
            :close="handleCloseCptDowntimesDialog"
            :id_place_cpt="currentSelectedIdPlaceCpt"
            :date="date"
        ></CptDowntimes>
        <DowntimesByReason
        ...
        ></DowntimesByReason>
        <PlaceReportsList :routerLink="'cptsReport'" :show="showReportsList" :close="closeReportsList" :id_technica_type = "1" :id_place="id_place" :date="date"></PlaceReportsList>
...
<script>
import {getMainData, storePlan, storeFact} from "@/api/cpts";
import {getPlaceForExcel} from "@/api/places";
import checkPermission from '@/utils/permission'; // Permission checking
import CptDowntimes from "./CptDowntimes.vue";
...
export default { 
    components: { CptDowntimes, DowntimesByReason, PlaceReportsList, SvodTable },
    props: {
        id_place: Number,
        date: String,
        updater: Number,
    ...
    data() {
        return {
            ...
            currentSelectedIdPlaceCpt: null, 
            showCptDowntimesDialog: false,
            ...
            cptsData: [], // Главный список по причинам
            reasonsColumns: {}, 
    ...
    methods: {
        ...
        async loadData() 
        {
            ...
            */
            ...
            this.cptsData = res.data.plans_facts.map(e => ({
            ...
            this.cptsData = this.cptsData.map(e => {
                ...
                /* const x = 0; // Регламентированный простой 
            ...
            const dayHours = 24*this.cptsData.length;
        ...
        changeVolumeView()
        {
            ...
            this.$nextTick(() => { this.$refs['cptsDatas'].doLayout(); })
        }, 
        changeDowntimeView() 
        {
        ...
        forgetValue(row, type) {
            if(row.id_place_cpt == this.savedFocusValue.id_place_cpt && type == this.savedFocusValue.type)
            {
        ...
        handleShowCptDowntimesDialog(id_place_cpt) 
        {
            console.log(id_place_cpt);
            this.currentSelectedIdPlaceCpt = id_place_cpt;
            this.showCptDowntimesDialog = true;
        },
        handleCloseCptDowntimesDialog()
        {
            this.showCptDowntimesDialog = false;
            this.currentSelectedIdPlaceCpt = null;
        },
        ...
        handleShowReasonDowntimesDialog(reason)
        {
            ...
            const day = this.cptsData.at(-1).downtimes[reason.key].dayDowntime;
            const since = this.cptsData.at(-1).downtimes[reason.key].sinceDowntime;
        ...
        async excelHeader() 
        {
            ...
            else if(this.excelConfig.datas != 1 && this.excelConfig.downtimesShort) 
            {
                ...
                merges.push(`A2:A3`, `B2:B3`);
            }
            else 
            {
                multiHeader.push(secondRow, thirdRow);
                merges.push(`A2:A4`, `B2:B4`);
            }
            
            let columnCounter = {
                firstRow: 2,
                secondRow: 2,
                thirdRow: multiHeader.length <= 1 ? 0 : 2,
                fourthRow: multiHeader.length == 2 ?  2 : 0,
            };
            ...
            if(this.excelConfig.volumes)
            {
                if(this.excelConfig.datas == 1) // если требуется полные данные (сутки, с нач месяца)
                {
                    ...
                    merges.push(`C2:J2`,  // Объём на 8 клеток
                                `C3:F3`,  // Объём за сутки на 4 клеток
                                `G3:J3`   // Объём с начала месяца на 4 клеток
                                );
                    if(!this.excelConfig.downtimesShort)
                    {
                        columnCounter.thirdRow  +=8;
                        columnCounter.fourthRow += 8;
                        merges.push(`C4:C5`, `D4:D5`, `E4:E5`, `F4:F5`, `G4:G5`, `H4:H5`,`I4:I5`,`J4:J5`); // на 2 строки
                    }
                }   
                else {
                    ...
                    merges.push(`C2:F2`); // Объём на 4 клеток 
                    if(!this.excelConfig.downtimesShort)
                    {
                        columnCounter.fourthRow += 4;
                        merges.push(`C2:F2`, `D3:D4`, `E3:E4`, `F3:F4`); // на 2 строки
                    }
                }
            }
        ...
        excelGetData() {
            const data = [];
            this.cptsData.forEach((e, i, self ) => {

                const currentCpt = [i+1, e.title];
                if(i == self.length - 1) 
                    currentCpt[0] = '';
                ...
                if(this.excelConfig.volumes) 
                {
                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                        currentCpt.push(e.plan, e.fact, e.differenceDay, e.percentDay)
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                        currentCpt.push(e.plan_month, e.fact_month, e.differenceMonth, e.percentMonth);
                }
                ...
                if(!this.excelConfig.downtimesShort)
                {
                    ...
                    planned.forEach(r => { 
                        if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1)  
                            currentCpt.push(e.downtimes[r].dayDowntime);
                        if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                            currentCpt.push(e.downtimes[r].sinceDowntime);
                    });
                    ...
                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1)
                        currentCpt.push(e.downtimes['summary'].dayDowntime);
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1)
                        currentCpt.push(e.downtimes['summary'].sinceDowntime);

                }
                else 
                {
                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                        currentCpt.push(e.downtimes['planned'].dayDowntime);
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1)
                        currentCpt.push(e.downtimes['planned'].sinceDowntime);

                    if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                        currentCpt.push(e.downtimes['unscheduled'].dayDowntime);
                    if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1)
                        currentCpt.push(e.downtimes['unscheduled'].sinceDowntime);
                ...
                if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                    currentCpt.push(e.workHour.day);
                if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                    currentCpt.push(e.workHour.since);
                if(this.excelConfig.datas == 2 || this.excelConfig.datas == 1) 
                    currentCpt.push(e.kio.day);
                if(this.excelConfig.datas == 3 || this.excelConfig.datas == 1) 
                    currentCpt.push(e.kio.since);

                data.push(currentCpt);
            });
            return data;
        },
