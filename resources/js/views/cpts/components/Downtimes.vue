<template>
    <div class="app-container">
        <h3>Остановки ЦПТ за сегодня {{ showTable ? "": "(скрыто)" }}
            ...
            <el-button 
                    ...
                    style="float: right;"
                    @click="openAdd"
                    v-if="checkPermission(['manage cpts'])"
        ...
        <div v-if="this.showTable">
            <el-form :model="downtimesFilter" label-position="top">
                <el-row :gutter="20">
                    ...
                    <el-col :span="4">
                        <el-form-item label="Фильтр по ЦПТ" label-position="top">
                            <el-select 
                                v-model="downtimesFilter.selectedCpt" 
                                filterable
                                value-key="id_cpt"
                                :filter-method="cptFilter"
                            >
                                <el-option class="cptOption all"
                                    :key="null"
                                    label="Все"
                                    :value="null"
                                >
        ...
        >
            ...
            <el-table-column :min-width="14" label="ЦПТ" prop="title_cpt" align="center"></el-table-column>
            ...
            <el-table-column :min-width="8"  label="Действия" align="center">
                <template slot-scope="scope">
                    <el-button v-if="!scope.row.end_time && checkPermission(['manage cpts'])"  type="info" icon="el-icon-edit" circle @click="openEdit(scope.row.id_downtime)"></el-button>
                    <el-button v-if="checkPermission(['manage cpts'])" type="danger" icon="el-icon-delete" circle @click="remove(scope.row.id_downtime)"></el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <AddDowntime  v-if="checkPermission(['manage cpts'])" :show="showAdd" :close="closeAdd" :placeId="id_place" :date="date"></AddDowntime>
        <EditDowntime  v-if="editingDowntimeId && checkPermission(['manage cpts'])" :show="showEdit" :close="closeEdit" :placeId="id_place" :date="date" :downtimeId="editingDowntimeId"></EditDowntime>
    </div>
</template>


<script>
import {getCptDowntimesDailiy, deleteDowntime} from '@/api/cpts';
...
export default {
    ...
    methods: {
        checkPermission,
        async loadDowntimes() {
            const { data } = await getCptDowntimesDailiy(this.date, this.id_place);
            ...
            const cpts = data.downtimes.map(d => ({
                id_cpt: d.id_cpt, 
                title: d.title_cpt,
                garage: d.garage,
            }));

            this.cptsList = cpts.filter((e,i,self) => i === self.findIndex((t) => t.id_cpt === e.id_cpt));
            this.cptsListFiltered = this.cptsList;
        ...
        paginate(p) 
        {
            if(this.downtimesFilter.selectedCpt || this.downtimesFilter.selectedReason || this.downtimesFilter.search.trim()){
                const s = this.downtimesFilter.search.trim();
                let filtered = this.downtimesList;

                if(this.downtimesFilter.selectedCpt)
                {
                    filtered = filtered.filter(r => r.id_cpt == this.downtimesFilter.selectedCpt.id_cpt);
                }
                ...
                if(s)
                {
                    filtered = filtered.filter(r => (r.code.indexOf(s) != -1 || r.garage.indexOf(s) != -1 || r.title_cpt.indexOf(s) != -1 || r.reason.indexOf(s) != -1));
                }
        ...
        cptFilter(search)
        {
            this.cptsListFiltered =  this.cptsList.filter(r => (r.garage.indexOf(search) != -1 || r.title.indexOf(search) != -1 ));
        }, 
        reasonFilter(search)
        {
...
.el-select-dropdown__item.cptOption.all { 
