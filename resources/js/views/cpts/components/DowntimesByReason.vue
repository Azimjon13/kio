    ...
    >
        <div class="form-container">
            ...
            <el-select v-model="downtimesFilter.id_technica_group" @change="filterDowntimes('reset')">
                ...
                <el-option v-for="group in cptGroupTypes" :value="group.id_technica_group" :label="group.title_technica_type"></el-option>
            </el-select>


            <el-select 
                    v-model="downtimesFilter.id_cpt" 
                ...
                >
                    <div class="autocomplete-list-item">
                            <span class="name-autocomplete">
                                <b>Наименование:</b> {{ cpt.title_cpt }}
                            </span>
                            <br>
            ...
            >
                ...
                <el-table-column label="Тип ЦПТ" prop="title_technica_type" :min-width="9" align="center"></el-table-column>
                <el-table-column label="ЦПТ" prop="title_cpt" :min-width="15" align="center"></el-table-column>
...
<script>
// import {downtimesByReasonPlace} from "@/api/cpts";
import {downtimesByReasonPlace} from "../../../api/cpts";
...
export default {
    ...
    methods: { 
        async loadData() {
            ...
            const groupsCpts = data.downtimes.map(r=>({id_technica_group: r.id_technica_group, title_technica_type: r.title_technica_type}));
            this.cptGroupTypes = groupsCpts.reduce((acc, curr, ind) => {
            ...
            const cpts = data.downtimes.map(r => ({id_cpt: r.id_cpt, title_cpt: r.title_cpt, title_technica_type: r.title_technica_type, id_technica_group: r.id_technica_group, garage: r.garage}));
            this.cptsList = cpts.reduce((acc, curr, ind) => {
                if(!acc.find(a => a.id_cpt === curr.id_cpt)){
                    acc.push(curr);
                }
                return acc;
            }, []);
        ...
        filterDowntimes(reset) {
            ...
            if(this.downtimesFilter.id_technica_group)
            {
                ...
                cpts = cpts.filter(e => e.id_technica_group == this.downtimesFilter.id_technica_group);
            }

            if(reset == 'reset')
            {
                this.downtimesFilter.id_cpt = null;
            }

            if(this.downtimesFilter.id_cpt) 
            {
                downtimes = downtimes.filter(d => d.id_cpt == this.downtimesFilter.id_cpt);
            }
            
            this.cptsListFiltered = cpts;
        ...
        cptFilter(search)
        {
            this.cptsListFiltered =  this.cptsList.filter(r => (r.garage.indexOf(search) != -1 || r.title_cpt.indexOf(search) != -1 ));
        }, 
        ...
        excelHeader() 
        {
            ...
            const fourthRow = ['','','','','','','','', '']; 
            const fivthRow = ['№ п/п', 'Тип ЦПТ', 'ЦПТ', 'Гараж №', 'Код', 'Описание кода простоя', 'Дата начала простоя', 'Дата окончания простоя', 'Длительность (в час)'];
            
            const merges = ['B1:C1',  // Первая строка Площадка на 2 колонки
                            'D1:E1',  // Первая строка ЦПТ на 2 колонки
