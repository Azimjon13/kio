...
class TruckFactModel extends Model
{
    use SoftDeletes;
    protected $table = 'facts_trucks';
    ...
    public static function excluded_facts_inmonth_inyear($date_day, $id_place){
        ...
        return DB::select("SELECT
            ...
            FROM facts_trucks f,
            (
                SELECT a.id_place_truck, MAX(a.date_fact) as 'maxDate'
                FROM facts_trucks a
                WHERE a.deleted_at is null AND a.id_place = :id_place
                GROUP BY a.id_place_truck
            ) f1
            WHERE f.date_fact = f1.maxDate  AND f.id_place_truck = f1.id_place_truck AND f.id_place_truck in ( SELECT
            pe.id
