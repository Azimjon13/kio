...
class LocomotivePlanModel extends Model
{
    use SoftDeletes;
    protected $table = 'plans_locomotives';
    ...
    public static function excluded_plans_inmonth_inyear($date_day, $id_place){
        ...
        return DB::select("SELECT
                ...
                FROM plans_locomotives p,
                (
                    SELECT a.id_place_locomotive, MAX(a.date_plan) as 'maxDate'
                    FROM plans_locomotives a
                    WHERE a.deleted_at is null AND a.id_place = :id_place
                    GROUP BY a.id_place_locomotive
                ) p1
                WHERE p.date_plan = p1.maxDate  AND p.id_place_locomotive = p1.id_place_locomotive AND p.id_place_locomotive in ( SELECT
                pe.id
