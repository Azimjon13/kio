...
class DrillDowntimeModel extends Model
{
    ...
    public static function downtime_plan_fact($date_day, $id_place){
        return DB::select("SELECT
            ...
            p.plan_year,
            f.fact_year
        FROM drills e, places_drills pe
                LEFT JOIN plans_drills p ON
                            pe.id = p.id_place_drill AND
                            p.date_plan = :date_day AND
                            p.deleted_at is null
                LEFT JOIN facts_drills f ON
                            pe.id = f.id_place_drill AND
    ...
    public static function downtime_daily_by_plan_fact_by_report($date_day, $id_report, $id_place, $id_place_drills){
        $shifts = PlaceShiftModel::where('id_place', '=', $id_place)->where('id_technica_type', '=', 2)->orderBy('shift')->get();
        $start_difference = $shifts?->first()?->difference;
        $end_difference = $shifts?->last()?->difference + $shifts?->last()?->duration? : 1440 + $shifts?->first()?->difference;
...
FROM  drills e, places p, places_drills pe, downtimes_drills de, codes c, reasons r
WHERE
	pe.id_drill = de.id_drill AND
    pe.id_place = de.id_place AND
    de.id_code = c.id AND
    c.id_reason = r.id AND
    pe.id_place = p.id AND pe.id_drill = e.id AND
...
FROM  places_drills pe,     reasons r , codes c,  downtimes_drills de
WHERE
	c.id_reason = r.id AND
    de.id_code = c.id AND
    de.id_place_drill = pe.id AND
    de.start_time < :shift_end AND
    ( de.end_time > :shift_month_start OR de.end_time is null ) AND
    c.deleted_at is null AND
    c.deleted_at is null AND
...
FROM places p, drills e, reasons r, technica_groups tg,  places_drills pe, codes c, downtimes_drills de
WHERE
		p.id = pe.id_place AND
		c.id_reason = r.id AND
		e.id = pe.id_drill AND
        de.id_place_drill = pe.id AND
        de.id_code = c.id AND
