...
class DsmFactModel extends Model
{
    use SoftDeletes;
    protected $table = 'facts_dsms';
    ...
    public static function excluded_facts_inmonth_inyear($date_day, $id_place){
        ...
        return DB::select("SELECT
            ...
            FROM facts_dsms f,
            (
                SELECT a.id_place_dsm, MAX(a.date_fact) as 'maxDate'
                FROM facts_dsms a
                WHERE a.deleted_at is null AND a.id_place = :id_place
                GROUP BY a.id_place_dsm
            ) f1
            WHERE f.date_fact = f1.maxDate  AND f.id_place_dsm = f1.id_place_dsm AND f.id_place_dsm in ( SELECT
            pe.id
            FROM places_dsms pe
