...
use App\Kio\Dsms\DsmModel;
use App\Kio\Dsms\DsmPlaceModel;
...
class DsmDowntimeModel extends Model
{
    use SoftDeletes;
    protected $table = 'downtimes_dsms';
    ...
    public function place_dsm(){
        return $this->belongsTo(DsmPlaceModel::class, 'id_place_dsm', 'id');
    }

    public static function downtime_plan_fact($date_day, $id_place){
        return DB::select("SELECT
            pe.id as 'id_place_dsm',
            pe.id_place,
            pe.id_dsm,
            e.title,
            pe.garage,
            ...
            p.plan_year,
            f.fact_year
        FROM dsms e, places_dsms pe
                LEFT JOIN plans_dsms p ON
                            pe.id = p.id_place_dsm AND
                            p.date_plan = :date_day AND
                            p.deleted_at is null
                LEFT JOIN facts_dsms f ON
                            pe.id = f.id_place_dsm AND
    ...
    public static function downtime_daily_by_plan_fact($date_day, $id_place, $id_place_dsms){
        ...
        foreach ($id_place_dsms as $pos => $id) {
            $arrPos[] = ":id_{$pos}";
            $arrBind["id_{$pos}"] = $id;
        }
        if(!empty($arrPos)){
            $query_part = " de.id_place_dsm IN (".implode(', ', $arrPos).")";
        }else{
            $query_part = " de.id_place_dsm IN ( null )";
        }
       return DB::select("SELECT
                                r.id as 'id_reason',
                                r.reason,
                            ...
                            FROM reasons r
                                    ...
                                    LEFT JOIN downtimes_dsms de   ON
                                        de.id_code = c.id  AND
                                        de.start_time <= :shift_end AND
                                        (de.end_time >= :shift_month_start OR de.end_time is null) AND
                                        ".$query_part." AND
                                        de.deleted_at is null
                            WHERE r.deleted_at IS NULL AND r.dsm = 1
                            GROUP BY r.id, r.reason, de.id_place_dsm, de.id_dsm,   r.type,  r.is_reported", $arrBind);
    }

    public static function downtime_daily_by_plan_fact_by_report($date_day, $id_report, $id_place, $id_place_dsms){
        $shifts = PlaceShiftModel::where('id_place', '=', $id_place)->where('id_technica_type', '=', 5)->orderBy('shift')->get();
        $start_difference = $shifts?->first()?->difference;
        $end_difference = $shifts?->last()?->difference + $shifts?->last()?->duration? : 1440 + $shifts?->first()?->difference;
...
FROM  dsms e, places p, places_dsms pe, downtimes_dsms de, codes c, reasons r
WHERE
	pe.id_dsm = de.id_dsm AND
    pe.id_place = de.id_place AND
    de.id_code = c.id AND
    c.id_reason = r.id AND
    pe.id_place = p.id AND pe.id_dsm = e.id AND
...
GROUP BY r.id, r.reason,  r.type,  r.is_reported", $arrBind);
    ...
    public static function downtime_by_dsm($date_day, $id_place_dsm){
        $id_place = DsmPlaceModel::find($id_place_dsm)->id_place;
        $shifts = PlaceShiftModel::where('id_place', '=', $id_place)->where('id_technica_type', '=', 5)->orderBy('shift')->get();
        $start_difference = $shifts?->first()?->difference;
        $end_difference = $shifts?->last()?->difference + $shifts?->last()?->duration? : 1440 + $shifts?->first()?->difference;
        ...
        $arrBind = ["shift_month_start" => $shift_month_start, "shift_end" => $shift_end, "id_place_dsm" => $id_place_dsm];
        return DB::select("SELECT
pe.id as 'id_place_dsm',
...
FROM  places_dsms pe,     reasons r , codes c,  downtimes_dsms de
WHERE
	c.id_reason = r.id AND
    de.id_code = c.id AND
    de.id_place_dsm = pe.id AND
    de.start_time < :shift_end AND
    ( de.end_time > :shift_month_start OR de.end_time is null ) AND
    c.deleted_at is null AND
    c.deleted_at is null AND
...
FROM places p, dsms e, reasons r, technica_groups tg,  places_dsms pe, codes c, downtimes_dsms de
WHERE
