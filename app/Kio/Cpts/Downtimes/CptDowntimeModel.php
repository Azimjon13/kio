...
use App\Kio\Cpts\CptModel;
use App\Kio\Cpts\CptPlaceModel;
...
class CptDowntimeModel extends Model
{
    use SoftDeletes;
    protected $table = 'downtimes_cpts';
    ...
    public function place_cpt(){
        return $this->belongsTo(CptPlaceModel::class, 'id_place_cpt', 'id');
    }

    public static function downtime_plan_fact($date_day, $id_place){
        return DB::select("SELECT
            pe.id as 'id_place_cpt',
            pe.id_place,
            pe.id_cpt,
            e.title,
            pe.garage,
            ...
            p.plan_year,
            f.fact_year
        FROM cpts e, places_cpts pe
                LEFT JOIN plans_cpts p ON
                            pe.id = p.id_place_cpt AND
                            p.date_plan = :date_day AND
                            p.deleted_at is null
                LEFT JOIN facts_cpts f ON
                            pe.id = f.id_place_cpt AND
    ...
    public static function downtime_daily_by_plan_fact($date_day, $id_place, $id_place_cpts){
        ...
        foreach ($id_place_cpts as $pos => $id) {
            $arrPos[] = ":id_{$pos}";
            $arrBind["id_{$pos}"] = $id;
        }
        if(!empty($arrPos)){
            $query_part = " de.id_place_cpt IN (".implode(', ', $arrPos).")";
        }else{
            $query_part = " de.id_place_cpt IN ( null )";
        }
       return DB::select("SELECT
                                r.id as 'id_reason',
                                r.reason,
                            ...
                            FROM reasons r
                                    ...
                                    LEFT JOIN downtimes_cpts de   ON
                                        de.id_code = c.id  AND
                                        de.start_time <= :shift_end AND
                                        (de.end_time >= :shift_month_start OR de.end_time is null) AND
                                        ".$query_part." AND
                                        de.deleted_at is null
                            WHERE r.deleted_at IS NULL AND r.cpt = 1
                            GROUP BY r.id, r.reason, de.id_place_cpt, de.id_cpt,   r.type,  r.is_reported", $arrBind);
    }

    public static function downtime_daily_by_plan_fact_by_report($date_day, $id_report, $id_place, $id_place_cpts){
        $shifts = PlaceShiftModel::where('id_place', '=', $id_place)->where('id_technica_type', '=', 3)->orderBy('shift')->get();
        $start_difference = $shifts?->first()?->difference;
        $end_difference = $shifts?->last()?->difference + $shifts?->last()?->duration? : 1440 + $shifts?->first()?->difference;
...
FROM  cpts e, places p, places_cpts pe, downtimes_cpts de, codes c, reasons r
WHERE
	pe.id_cpt = de.id_cpt AND
    pe.id_place = de.id_place AND
    de.id_code = c.id AND
    c.id_reason = r.id AND
    pe.id_place = p.id AND pe.id_cpt = e.id AND
...
GROUP BY r.id, r.reason,  r.type,  r.is_reported", $arrBind);
    ...
    public static function downtime_by_cpt($date_day, $id_place_cpt){
        $id_place = CptPlaceModel::find($id_place_cpt)->id_place;
        $shifts = PlaceShiftModel::where('id_place', '=', $id_place)->where('id_technica_type', '=', 3)->orderBy('shift')->get();
        $start_difference = $shifts?->first()?->difference;
        $end_difference = $shifts?->last()?->difference + $shifts?->last()?->duration? : 1440 + $shifts?->first()?->difference;
        ...
        $arrBind = ["shift_month_start" => $shift_month_start, "shift_end" => $shift_end, "id_place_cpt" => $id_place_cpt];
        return DB::select("SELECT
pe.id as 'id_place_cpt',
...
FROM  places_cpts pe,     reasons r , codes c,  downtimes_cpts de
WHERE
	c.id_reason = r.id AND
    de.id_code = c.id AND
    de.id_place_cpt = pe.id AND
    de.start_time < :shift_end AND
    ( de.end_time > :shift_month_start OR de.end_time is null ) AND
    c.deleted_at is null AND
    c.deleted_at is null AND
...
FROM places p, cpts e, reasons r, technica_groups tg,  places_cpts pe, codes c, downtimes_cpts de
WHERE
