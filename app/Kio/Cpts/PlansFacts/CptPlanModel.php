...
class CptPlanModel extends Model
{
    use SoftDeletes;
    protected $table = 'plans_cpts';
    ...
    public static function excluded_plans_inmonth_inyear($date_day, $id_place){
        ...
        return DB::select("SELECT
                ...
                FROM plans_cpts p,
                (
                    SELECT a.id_place_cpt, MAX(a.date_plan) as 'maxDate'
                    FROM plans_cpts a
                    WHERE a.deleted_at is null AND a.id_place = :id_place
                    GROUP BY a.id_place_cpt
                ) p1
                WHERE p.date_plan = p1.maxDate  AND p.id_place_cpt = p1.id_place_cpt AND p.id_place_cpt in ( SELECT
                pe.id
                FROM places_cpts pe
